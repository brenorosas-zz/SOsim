...
struct DISCO
{
    ...
    void remove(Pagina pagina)
    {
        for (int i = 0; i < disco.size(); i++)
        {
            if (pagina == disco[i])
            {
                cout << "Removendo do disco a pagina " << pagina.idPagina << ' ' << pagina.idProcessoAssociado << endl;
...
struct MaquinaVirtual
{
    ...
    void page(vector<Pagina> &paginas)
    {
        ...
        {
            cout << "paginando pagina " << pagina.idPagina << ' ' << pagina.idProcessoAssociado << endl;
            if (!ram.existe(pagina))
            {
                cout << "Buscando pagina " << pagina.idPagina << ' ' << pagina.idProcessoAssociado << " no disco" << endl;
                Pagina aux;
                if (paginacao == "FIFO")
                ...
                {
                    cout << "Removeu a pagina " << aux.idPagina << ' ' << aux.idProcessoAssociado << " da Ram" << endl;
                    disco.add(aux);
                }
                tempo++;
                disco.remove(pagina);
    ...
    {
        cout << "Preparando processo " << processo.idProcesso << " para execução" << endl;
